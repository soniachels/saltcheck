...
export async function generateCashflowPdf({ entry, startDate, endDate, fmt, userName }: CashflowParams) {
  ...
  const sum = (arr: any[]) => arr.reduce((s, x) => s + (x.amount || 0), 0);
  const paidBills = bills.filter((b: any) => b.paid);
  const recvIncome = income.filter((i: any) => i.received);
  const net = sum(recvIncome) - sum(paidBills) - sum(doom) + sum(soft);
  ...
  <html><head><meta name="viewport" content="width=device-width" /><style>
    ...
    <div class="summary">
      <div class="stat"><div class="label">RECEIVED</div><div class="val" style="color:#1a7f37">${fmt(sum(recvIncome))}</div></div>
      <div class="stat"><div class="label">PAID OUT</div><div class="val" style="color:#C4191E">${fmt(sum(paidBills))}</div></div>
